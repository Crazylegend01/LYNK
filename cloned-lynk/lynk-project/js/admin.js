...
async function loadReports() {
  ...
  try {
    // Try with orderBy first (requires a composite Firestore index)
    let snap;
    try {
      snap = await getDocs(query(collection(db, 'reports'), where('status','==','open'), orderBy('createdAt','desc'), limit(25)));
    } catch (indexErr) {
      // Composite index not created yet — fall back to unordered query
      if (indexErr.code === 'failed-precondition' || indexErr.message?.includes('index')) {
        snap = await getDocs(query(collection(db, 'reports'), where('status','==','open'), limit(25)));
        list.insertAdjacentHTML('beforeend', `
          <div class="lynk-card p-3 mb-3 text-xs" style="background:rgba(245,158,11,0.08);border:1px solid rgba(245,158,11,0.3);color:#f59e0b">
            ⚠️ To enable newest-first sorting, create a composite Firestore index on <code>reports</code>: <strong>status ASC, createdAt DESC</strong>.
            Open the Firebase Console → Firestore → Indexes and add it.
          </div>`);
      } else { throw indexErr; }
    }
    ...
    list.innerHTML = `<div class="lynk-card p-4 text-sm" style="color:var(--text-muted)">Error loading reports: ${escHtml(e.message)}<br><span class="text-xs">Make sure your Firestore security rules allow admin read on the reports collection, and that you are signed in as a Super Admin.</span></div>`;
  }
}
