...
async function loadStats() {
  try {
    ...
    document.getElementById('stat-active').textContent = Math.floor(Math.random() * 200 + 50); // Approximation
    ...
    if (anaDau) anaDau.textContent = Math.floor(Math.random()*500+200);
    const anaPd = document.getElementById('ana-posts-day');
    if (anaPd) anaPd.textContent = Math.floor(Math.random()*100+20);
    const anaMsg = document.getElementById('ana-messages');
    if (anaMsg) anaMsg.textContent = Math.floor(Math.random()*800+100);
  } catch (e) {
...
function initCharts() {
  ...
  if (actCtx) {
    ...
    chartInstances['activity'] = new Chart(actCtx, {
      type: 'doughnut',
      data: {
        labels: ['Posts', 'Comments', 'Messages', 'Reactions', 'Friend Requests'],
        datasets: [{
          data: [35, 20, 25, 15, 5],
          backgroundColor: [g1, g2, g3, '#f59e0b', '#ec4899'],
          borderWidth: 0,
          hoverOffset: 8
        }]
      },
      options: { responsive: true, maintainAspectRatio: true, plugins: { legend: { labels: { color: textColor, font: { family: 'Inter', size: 11 } } } } }
    });
  }
  ...
  if (facCtx) {
    ...
    chartInstances['faculty'] = new Chart(facCtx, {
      type: 'bar',
      data: {
        labels: ['Science', 'Arts', 'Engineering', 'Medicine', 'Law', 'Business'],
        datasets: [{
          label: 'Posts',
          data: [65, 42, 88, 31, 45, 72],
          backgroundColor: [g1, g2, g3, '#f59e0b', '#ec4899', '#22c55e'],
          borderRadius: 8,
          borderSkipped: false
        }]
      },
  ...
  if (trendCtx) {
    ...
    chartInstances['30day'] = new Chart(trendCtx, {
      type: 'line',
      data: {
        labels,
        datasets: [{
  ...
  if (ctCtx) {
    ...
    chartInstances['ctypes'] = new Chart(ctCtx, {
      type: 'pie',
      data: {
        labels: ['Text Posts', 'Image Posts', 'Video Posts', 'Polls'],
        datasets: [{
          data: [55, 25, 12, 8],
          backgroundColor: [g1, g2, g3, '#f59e0b'],
          borderWidth: 0
        }]
      },
      options: { responsive: true, maintainAspectRatio: true, plugins: { legend: { labels: { color: textColor, font: { family: 'Inter', size: 11 } } } } }
...
async function loadUsersTable(filter = 'all', search = '') {
  ...
  if (!tbody) return;
  tbody.innerHTML = '<tr><td colspan="5" class="p-8 text-center" style="color:var(--text-muted)"><div class="spinner inline-block w-6 h-6 border-2 rounded-full" style="border-color:var(--grad-1) transparent"></div></td></tr>';
  ...
  if (filter === 'unverified') q = query(collection(db, 'users'), where('emailVerified', '==', false), limit(50));

  const snap = await getDocs(q);
  tbody.innerHTML = '';
  let rows = snap.docs;
  if (search) rows = rows.filter(d => (d.data().displayName||'').toLowerCase().includes(search.toLowerCase()) || (d.data().email||'').toLowerCase().includes(search.toLowerCase()));

  if (rows.length === 0) {
    tbody.innerHTML = '<tr><td colspan="5" class="p-8 text-center text-sm" style="color:var(--text-muted)">No users found.</td></tr>';
    return;
  }
  ...
  rows.forEach(d => {
    ...
    tbody.insertAdjacentHTML('beforeend', `
      <tr class="border-b hover:bg-opacity-50 transition-colors" style="border-color:var(--border)">
        ...
        <td class="p-4 text-sm" style="color:var(--text-secondary)">${u.university || '—'}</td>
        ...
        <td class="p-4">
          <div class="flex gap-1">
            ...
            ${!isSuspended
              ...
              : `<button onclick="suspendUser('${d.id}', '${u.displayName?.replace(/'/g,"\\'")}', false)" class="lynk-btn lynk-btn-ghost py-1 px-2 rounded-lg text-xs" style="color:#22c55e">✅ Unsuspend</button>`
            }
          </div>
        </td>
      </tr>`);
...
window.suspendUser = async (uid, name, suspend) => {
  ...
  document.getElementById('user-action-title').textContent = suspend ? 'Suspend User' : 'Unsuspend User';
  document.getElementById('user-action-desc').textContent = `${suspend ? 'Suspend' : 'Restore'} account for ${name}? They will ${suspend ? 'not' : 'now'} be able to access LYNK.`;
...
async function loadContentList(filter = 'all') {
  ...
  if (filter === 'reported') q = query(collection(db, 'reports'), where('status', '==', 'open'), limit(20));
  const snap = await getDocs(q);
  list.innerHTML = '';
      ...
      <div class="lynk-card p-5">
        <div class="flex items-start justify-between gap-4">
          ...
          <div class="flex gap-2 flex-shrink-0">
            <button onclick="removePost('${d.id}')" class="lynk-btn lynk-btn-ghost text-xs py-1 px-3 rounded-lg" style="color:#ef4444">🗑 Remove</button>
          </div>
        </div>
      </div>`);
  });
...
async function loadSecurityLogs() {
  ...
  if (snap.empty) {
    // Generate sample logs from real user signins
      ...
      container.insertAdjacentHTML('beforeend', `
        <div class="flex items-center justify-between p-4 text-sm">
          <div class="flex items-center gap-3">
            ...
            <div>
              <span class="font-medium">${u.displayName || 'User'}</span>
  ...
  snap.docs.forEach(d => {
    ...
    container.insertAdjacentHTML('beforeend', `
      <div class="flex items-center justify-between p-4 text-sm border-b" style="border-color:var(--border)">
        <div class="flex items-center gap-3">
          <span style="color:${colors[l.type]||'#94a3b8'}">●</span>
    ...
    list.insertAdjacentHTML('beforeend', `
      <div class="lynk-card p-5">
        <div class="flex items-center justify-between">
          <div>
            <p class="font-semibold text-sm mb-1">🚨 ${r.reason || 'User Report'}</p>
            <p class="text-xs" style="color:var(--text-muted)">Post ID: ${r.postId || '—'} · ${ts}</p>
          </div>
          <div class="flex gap-2">
            <button onclick="resolveReport('${d.id}','dismissed')" class="lynk-btn lynk-btn-secondary text-xs py-1.5 px-3 rounded-lg">Dismiss</button>
            <button onclick="resolveReport('${d.id}','actioned')" class="lynk-btn text-xs py-1.5 px-3 rounded-lg" style="background:#ef444422;color:#ef4444">Remove Post</button>
          </div>
        </div>
      </div>`);
  });
...
window.resolveReport = async (reportId, action) => {
  ...
  if (action === 'actioned' && snap.data()?.postId) {
    await deleteDoc(doc(db, 'posts', snap.data().postId)).catch(() => {});
  }
...
async function loadCommunities() {
  ...
  list.innerHTML = '';
  if (snap.empty) { list.innerHTML = '<div class="lynk-card p-6 col-span-full text-center text-sm" style="color:var(--text-muted)">No communities yet. Communities are auto-created when users sign up.</div>'; return; }
  snap.docs.forEach(d => {
    const c = d.data();
    list.insertAdjacentHTML('beforeend', `
      <div class="lynk-card p-5">
        <div class="flex items-center gap-3 mb-3">
          <span class="text-2xl">${c.type === 'faculty' ? '🏛️' : '📚'}</span>
          <div>
            <p class="font-semibold text-sm">${c.name}</p>
            <p class="text-xs" style="color:var(--text-muted)">${c.type} · ${c.university||''}</p>
          </div>
        </div>
        <div class="flex items-center justify-between text-sm">
          <span style="color:var(--text-secondary)">👥 ${c.memberCount||0} members</span>
          <span class="lynk-badge" style="background:var(--bg-card-hover);color:var(--text-muted)">${c.type}</span>
        </div>
      </div>`);
  });
}
...
async function loadAdminsList() {
  ...
  if (snap.empty) {
    list.innerHTML = '<p class="text-sm" style="color:var(--text-muted)">No sub-admins assigned yet.</p>';
    return;
  }
  list.innerHTML = '';
  snap.docs.forEach(d => {
    ...
    list.insertAdjacentHTML('beforeend', `
      <div class="flex items-center gap-2">
        ...
        <div class="flex-1 min-w-0">
          ...
          <p class="text-xs truncate" style="color:var(--text-muted)">${u.adminRole}</p>
        </div>
        ${currentUserData.adminRole === 'super' ? `<button onclick="revokeAdmin('${d.id}')" class="text-xs" style="color:#ef4444">Revoke</button>` : ''}
      </div>`);
  });
}
